function zmn = makeZmnOffset(numPerSide,separation)
...
%    square segments.  The plates are separated in the z direction by
%    'separation' units.  In this case the top plate is offset a tenth
%    of the unit distance in both the x and y directions relative to
%    the bottom plate.
...
for m=1:2*numPerPlate
  for n=m+1:2*numPerPlate
    ...
    zmn(m,n) = del2/sqrt(((i-ip)*del+0.1*(k-kp))^2 + ...
			 ((j-jp)*del+0.1*(k-kp))^2 + ...
			 (separation*(k-kp))^2);
  end
end
